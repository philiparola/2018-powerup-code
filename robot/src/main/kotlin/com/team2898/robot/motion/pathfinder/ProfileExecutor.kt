...
class ProfileExecutor(val profile: Deferred<Pair<Trajectory, Trajectory>>) {
    ...
    fun execute(motorCommand: (Double, Double) -> Unit) {
        println("ProfileExecutor @execute() line 28")
        job = go {
            println("ProfileExecutor @execute() line 30")
            println()
            started = true
        ...
        Drivetrain.masters {
            ...
            println("ProfileExecutor @execute() line 45")
            ...
            for (i in 0 until completedProfile.first.segments.size - 1) {
                println(i)
                val leftSegment = completedProfile.first.segments[i]
                val rightSegment = completedProfile.second.segments[i]
...
//                    Logger.logData("profile executor", "right target acc", rightSegment.acceleration)
                    ...
                    Logger.logInfo("profile executor", LogLevel.INFO,"left segment acc ${ leftSegment.acceleration}")
                    Logger.logInfo("profile executor", LogLevel.INFO,"right segment pos ${rightSegment.position}")
                    Logger.logInfo("profile executor", LogLevel.INFO,"right segment vel ${rightSegment.velocity}")
                    Logger.logInfo("profile executor", LogLevel.INFO,"right segment acc ${rightSegment.acceleration}")
